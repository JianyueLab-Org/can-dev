...
`/docs` 是**接口文档** —— 网络对外开放的那几个端点，连同统一登录的完整说明。
**要登录**：读它的人是来注册应用的，而注册应用本来就需要一个成员账号。它原来在
主站的 `ceruleanavi.net/developers`，那个地址现在是一条 301 —— 指向本站**首页**
而不是这一页，好让沿旧地址过来的未登录访客落在一页能说明情况、带着登录入口的页
面上，而不是一堵登录墙。
